...
For example, an AR(1) would be a “first order autoregressive process.” The outcome variable in a first order AR process at some point in time ![](https://latex.codecogs.com/svg.latex?t) is related only to time periods that are one period apart (i.e. the value of the variable at ![](https://latex.codecogs.com/svg.latex?t%20-%201)). An AR(p) model can be denoted as below.
